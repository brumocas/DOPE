...
colors = {
    ...
    'axis_x': (0, 0, 255),        # Red for X-axis
    'axis_y': (0, 255, 255),      # Yellow for Y-axis
    'axis_z': (255, 255, 0)       # Cyan for Z-axis
}
...
def draw_cuboid(image, cuboid_coords, name, debug=False):
    ...
    # Draw the Z-axis (up direction) from centroid
    z_axis_end = (centroid[0], centroid[1] - 50)
    cv2.arrowedLine(image, tuple(centroid), z_axis_end, colors['axis_z'], 2, tipLength=0.3)
    cv2.putText(image, 'Z', z_axis_end, cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors['axis_z'], 2)
...
def draw_cuboid(image, cuboid_coords, name, debug=False):
    ...
    # Draw the Y-axis (from point 5 to 6) from centroid
    mid_point_56 = ((image_coords[6][0] + image_coords[5][0]) // 2, 
                    (image_coords[6][1] + image_coords[5][1]) // 2)
    cv2.arrowedLine(image, tuple(centroid), mid_point_56, colors['axis_y'], 2, tipLength=0.3)
    cv2.putText(image, 'Y', mid_point_56, cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors['axis_y'], 2)
